        ...
        public async Task<ActionResult<Blog>> PostBlog([FromForm] UploadBlog upload)
        {
            ...
            else
            {
                ...
                blog = new Blog()
                {
                    ...
                    ImageBlogs = new List<ImageBlog>()  // Khởi tạo danh sách ImageBlogs
                };
                ...
                for (int i = 0; i < file.Length; i++)
                {
                    if (file[i].Length > 0 && file[i].Length < 2 * 1024 * 1024)
                    {
                        ...
                        var existingImage = await _context.ImageBlogs
                            .FirstOrDefaultAsync(img => img.ImageName == file[i].FileName && img.FkBlogId == blog.Id);

                        if (existingImage == null)
                        {
                            var image = new ImageBlog()
                            {
                                ImageName = await FileHelper.UploadImageAndReturnFileNameAsync3(null, upload, "blog", upload.files.ToArray(), i),
                                FkBlogId = blog.Id,
                                IsActive = true,
                                IsDelete = false,
                                CreatedDate = DateTime.Now,
                            ...
                            // Thêm ảnh vào navigation property của Blog
                            blog.ImageBlogs.Add(image); // Thêm vào danh sách ImageBlogs của Blog
                        }
                    }
                }
            }
            ...
            // Tín hiệu lại cho tất cả client nếu cần
            //await _hubContext.Clients.All.BroadcastMessage();

    ...
    public class SanPhamsController : Controller
    {
        ...
        public async Task<ActionResult> ListLikeSanPham(UserLike userlike)
        {
            var resuft = _context.UserLikes.Where(d => d.IdUser == userlike.IdUser).Select(
                d => new SanPhamLike
                {
                    ...
                    gia = (decimal)_context.SanPhams.Where(s => s.Id == d.IdSanPham).Select(s => s.KhuyenMai).FirstOrDefault(),
        ...
        public async Task<ActionResult<SanPham>> PostSanPham([FromForm] UploadSanpham upload)
        {
            ...
            if (upload.Id > 0)
            {
                ...
                ////lưu ctkm 
                //if (upload.Id_CTKM != null && upload.Id_CTKM!=0)
                //{
                //    sanpham.KhuyenMai = upload.KhuyenMai;
                //    var query = await  _context.SanPhamCTKM.Where(c => c.SanPhamId == upload.Id).ToListAsync();
                //    if (query.Any())
                //    {
                //        _context.SanPhamCTKM.RemoveRange(query);
                //    }
                //    if (upload.Id_CTKM > 0)
                //    {
                //        var ctkm = await _context.CTKMs.Where(c => c.Id == upload.Id_CTKM).FirstOrDefaultAsync();
                //        if (ctkm.DiscountType == "1")
                //        {
                //            sanpham.KhuyenMai = upload.GiaBan - upload.GiaBan*ctkm.DiscountValue/100;
                            
                //        }
                //        else if (ctkm.DiscountType == "2")
                //        {
                //            sanpham.KhuyenMai = upload.GiaBan - ctkm.DiscountValue;
                //        }
                //        if (sanpham.KhuyenMai < 0)
                //        {
                //            sanpham.KhuyenMai = upload.GiaNhap;
                //        }
                //        var spctkm = new SanPhamCTKM
                //        {
                //            CTKMId = upload.Id_CTKM ?? 0,
                //            SanPhamId = upload.Id,
                //        };
        ...
        [HttpGet("CTKM/{id}")]
        public async Task<ActionResult<CTKMs>> GetCTKM(int id)
        {
            var get=  await _context.SanPhamCTKM
                    .Include(s => s.CTKM)  
                    .Where(s => s.SanPhamId == id)
                    .Select(s => s.CTKM)    
                    .FirstOrDefaultAsync();

            return Ok(Result<CTKMs>.Success(get));
        }
        ...
        [HttpPost("loc")]
        public async Task<IActionResult> Filter([FromBody] JObject json)
        {
            var mau = json.GetValue("mausac")?.ToString();
            var loai = json.GetValue("loai")?.ToString();
            var thapStr = json.GetValue("thap")?.ToString();
            var caoStr = json.GetValue("cao")?.ToString();

            decimal? thap = null, cao = null;
            if (decimal.TryParse(thapStr, out var t)) thap = t;
            if (decimal.TryParse(caoStr, out var c)) cao = c;
            ...
            {
                var idloai = _context.Loais.Where(l => l.Ten == loai).Select(l => l.Id).FirstOrDefault();
                query = query.Where(sp => sp.Id_Loai == idloai);
            }

            // Lọc theo khoảng giá nếu có
            if (thap.HasValue)
                query = query.Where(sp => sp.KhuyenMai >= thap.Value);
            if (cao.HasValue)
                query = query.Where(sp => sp.KhuyenMai <= cao.Value);

            // Lọc theo màu nếu có
            if (!string.IsNullOrEmpty(mau))
            {
                var list_id_mau = _context.MauSacs
                    .Where(d => d.MaMau == mau)
                    .Select(d => d.Id.ToString()) 
                    .ToList();

                var list_spbienthe_theomau = _context.SanPhamBienThes
                    .Where(d => list_id_mau.Contains((d.Id_Mau.ToString())))
                    .Select(d => d.Id_SanPham)
                    .Distinct()
                    .ToList();

                query = query.Where(sp => list_spbienthe_theomau.Contains(sp.Id));
            }

            var result = await query.Select(s => new SanPhamLoaiThuongHieu
            {
                Id = s.Id,
                Ten = s.Ten,
                GiaBan = s.GiaBan,
            ...
            return Ok(Result<IEnumerable<SanPhamLoaiThuongHieu>>.Success(result));
        }

...
namespace API.Controllers
{
    ...
    public class SanPhamBienThesController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<GiaSanPhamMauSacSanPhamSize>>> GetSanPhamBienThes(string key, int offset, int limit = 20, string sortField = "", string sortOrder = "asc")
        {
            var query = from g in _context.SanPhamBienThes
                        ...
                        select new GiaSanPhamMauSacSanPhamSize
                        {
                            ...
                            MaLo = _context.SanPhamKho
                        .Where(spk => spk.SanPhamBienTheId == g.Id && spk.IsDelete==false)
                        .OrderBy(spk => spk.Id) // hoặc theo CreatedDate nếu muốn
                        .Select(spk => spk.MaLo)
                        .FirstOrDefault(),

                            // 🔻 Lấy IdKho tương tự (nếu muốn)
                            Id_Kho = _context.SanPhamKho
                        .Where(spk => spk.SanPhamBienTheId == g.Id && spk.IsDelete == false)
                        .OrderBy(spk => spk.Id)
                        .Select(spk => spk.KhoId)
                        .FirstOrDefault()
                        };
        ...
        public async Task<ActionResult<SanPhamBienThe>> PostSanPhamBienThe([FromForm] UploadSanPhamBienThe upload)
        {
            ...
            //    return Ok();
            SanPhamBienThe spbt;
            var query = await _context.SanPhamBienThes.Where(p => p.Id_SanPham == upload.SanPhamId && p.Id_Mau == upload.MauId && p.SizeId == upload.SizeId).AnyAsync();
            if(query)
            {
                return Ok(Result<SanPhamBienThe>.Error("Biến thể đã tồn tại"));
            }
            if (upload.Id > 0)
            {

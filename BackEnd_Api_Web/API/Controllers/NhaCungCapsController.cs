...
namespace API.Controllers
{
    ...
    public class NhaCungCapsController : ControllerBase
    {
        ...
        [HttpGet("GetShort")]
        public async Task<ActionResult<object>> GetShort()
        {
            var ncc = await _context.NhaCungCaps.Select(s => new { s.Ten, s.Id }).ToListAsync();
            return Ok(Result<object>.Success(ncc));
        }

...
namespace API.Controllers
{
    ...
    public class RoleController : Controller
    {
        ...
        public async Task<IActionResult> GetUserRole(string userId)
        {
            ...
            return Ok(Result<object>.Success(roles, roles.Count(), ""));
        }

        [HttpPost("AssignUserRoles")]
        public async Task<IActionResult> AssignUserRoles([FromBody] UserRolesDto form)
        {
            ...
            using (var transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                try
                {
                    var currentRoles = await _Db.UserRoles.Where(ur => ur.UserId == form.UserId).ToListAsync();
                    if (currentRoles.Any())
                    {
                    ...
                    if (!form.RoleIds.Any())
                    {
                        ...
                        return Ok(Result<object>.Success(null,1,"Đã xóa hết vai trò của người dùng."));
                    }

                    var validRoles = await _Db.Roles.Where(r => form.RoleIds.Contains(r.Id)).Select(r => r.Id).ToListAsync();

                    if (!validRoles.Any())
                    {
                        return NotFound("Không có vai trò nào hợp lệ.");
                    }

                    var userRoles = validRoles.Select(roleId => new IdentityUserRole<string>
                    {
                        UserId = form.UserId,
                        RoleId = roleId
                    }).ToList();
                    ...
                    transaction.Complete();
                    return Ok(Result<object>.Success(null, 1, $"Đã cập nhật {userRoles.Count} vai trò mới."));
                }
                catch (Exception ex)
                {
